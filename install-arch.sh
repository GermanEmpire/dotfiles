...
if [[ $warning = "y" || $warning = "Y" ]]; then
  ...
  yay --noconfirm --needed -q -S bspwm sxhkd conky dunst polybar network-manager-applet volumeicon feh \
    ranger flameshot brightnessctl pulsemixer chezmoi make libx11 bash-completion starship eza \
    shell-color-scripts-git bat rofi xclip sed zafiro-icon-theme thunar gvfs gvfs-mtp thunar-volman \
    xfce4 xarchiver thunar-archive-plugin thunar-media-tags-plugin tumbler ffmpegthumbnailer 
